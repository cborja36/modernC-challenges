...
void merge(double arr[], size_t l, size_t m, size_t r, double working_arr[]) {
  size_t i = l, j = m;
...
void MergeSort(double arr[], size_t l, size_t r, double working_arr[]) {
  if (r - l <= 1) return;

  size_t m = (l + r) / 2;
  MergeSort(arr, l, m, working_arr);
  MergeSort(arr, m, r, working_arr);

  merge(arr, l, m, r, working_arr);
}
